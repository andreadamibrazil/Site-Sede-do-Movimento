...
export default function LoginPage() {
  async function handleGoogleLogin() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/painel/auth/callback`,
      },
    })
  }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 w-full max-w-sm text-center space-y-6">
        <div>
          ...
          <h1 className="text-xl font-semibold text-gray-900 mt-1">Sistema de gestão</h1>
        </div>
        <button
        ...
        <p className="text-xs text-gray-400">
          Acesso restrito à equipe da Sede do Movimento
        </p>
      </div>
    </div>
  )
