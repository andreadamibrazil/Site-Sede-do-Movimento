...
//   0 min   → secretaria: alerta imediato / professor: importância da chamada
...
// GET = chamado pelo Vercel Cron / POST = chamado manualmente
...
async function handler(req: NextRequest) {
  ...
  const { data: aulas } = await (sb as any)
    .from('aulas')
    .select('id, data, hora_inicio, hora_fim, status, professor_id, professores(nome, celular), turmas(nome)')
  ...
  for (const aula of (aulas ?? [])) {
    const prof = (aula as any).professores
    ...
    for (const janela of JANELAS) {
      ...
      const msgProf = MSG_PROFESSOR[janela.tipo]?.(prof.nome, turma?.nome ?? 'sua turma', horaFormatada)
      if (!msgProf) continue
      ...
      // Alerta imediato para secretaria quando aula termina
  ...
  for (const sub of (subsSemSubstituto ?? [])) {
    ...
    // Verifica se já existe reposição para esta substituição
    ...
    if (!repExist) {
      // Cria reposição e notifica secretaria + professor
      ...
      // Notifica secretaria imediatamente
      ...
      // Acompanhamento: lembra professor a cada dia se prazo se aproxima
      ...
      // Prazo expirado → marca expirada + alerta secretaria
      if (hojeStr > prazoStr) {
