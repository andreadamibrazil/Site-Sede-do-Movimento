...
export default async function HomePage() {
  ...
  const galleryAlbums = (galleryData as { photos: { img: SanityImageSource }[] }[] | null) ?? [];
  return (
    <>
                ...
                {galleryPhotos.map((photo, i) => (
                  <ScrollReveal key={i} delay={i * 0.04}>
                    <Link href="/galerias/fotos" className={`group relative overflow-hidden rounded-lg block ${i === 0 ? "col-span-2 row-span-2 aspect-square" : "aspect-square"}`}>
                      <Image src={urlFor(photo.img).width(600).height(600).url()} alt="" fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 640px) 50vw, 25vw" />
