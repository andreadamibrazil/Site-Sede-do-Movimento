...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      {/* ── 9. BLOG PREVIEW ──────────────────────────────────────────────────── */}
      <section id="blog" className="section-padding bg-[#F9F8FC]">
        <div className="container-main">
          {/* Header */}
          <div className="flex items-end justify-between mb-10 md:mb-12">
            <div>
              <p className="text-brand-purple-500 font-semibold text-[11px] uppercase tracking-[0.15em] mb-3">
                Blog
              </p>
              <h2 className="text-[1.9rem] sm:text-[2.2rem] font-bold text-gray-900 leading-[1.18]">
                Novidades e conteúdo
              </h2>
            </div>
            <Link
              href="/blog"
              className="hidden md:flex items-center gap-1.5 text-brand-purple-600 font-semibold text-sm hover:gap-3 transition-all"
            >
              Ver todos <ArrowRight size={15} />
            </Link>
          </div>
          ...
          {recentPosts.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 xl:gap-10">

              {/* Featured post — large left */}
              <ScrollReveal>
                <Link href={`/blog/${recentPosts[0].slug}`} className="group block">
                  <div className="aspect-[3/2] rounded-2xl overflow-hidden relative mb-6 bg-gray-100 shadow-sm">
                    {recentPosts[0].coverImage ? (
                      ...
                      <div className="w-full h-full bg-gradient-tint flex items-center justify-center">
                        <span className="text-brand-purple-600/30 text-lg font-bold">{recentPosts[0].title}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <h3 className="text-2xl sm:text-[1.7rem] font-bold text-gray-900 mb-3 leading-snug group-hover:text-brand-purple-600 transition-colors duration-200">
                    {recentPosts[0].title}
                  </h3>
                  {recentPosts[0].excerpt && (
                    <p className="text-gray-500 text-[0.95rem] leading-relaxed line-clamp-3 mb-5">
                      {recentPosts[0].excerpt}
                    </p>
                  )}
                  ...
                  <span className="inline-flex items-center gap-1.5 text-brand-purple-600 font-semibold text-sm group-hover:gap-3 transition-all duration-200">
                    Ler artigo <ArrowRight size={14} />
                  </span>
                </Link>
              </ScrollReveal>

              {/* Side column — 2 vertical cards + CTA */}
              <div className="flex flex-col gap-4">
                {recentPosts.slice(1, 3).map((post, i) => (
                  <ScrollReveal key={post._id} delay={(i + 1) * 0.1}>
                    <Link
                      href={`/blog/${post.slug}`}
                      className="group block bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200"
                    >
                      <div className="aspect-[16/9] overflow-hidden relative bg-gray-100">
                        {post.coverImage ? (
                          ...
                          <div className="w-full h-full bg-gradient-tint" />
                        )}
                      </div>
                      <div className="p-4">
                        <Badge color="primary" variant="subtle" size="xs" className="mb-2">
                          {post.category}
                        </Badge>
                        <h3 className="font-bold text-gray-900 text-[0.9rem] leading-snug line-clamp-2 group-hover:text-brand-purple-600 transition-colors duration-200 mb-2">
                          {post.title}
                        </h3>
                        <p className="text-xs text-gray-400">{formatDate(post.publishedAt)} · {post.readingTime} min</p>
                      </div>
                    </Link>
                  </ScrollReveal>
                ))}
                ...
                <ScrollReveal delay={0.3}>
                  <Link href="/blog">
                    <Button variant="ghost" size="md" fullWidth rightIcon={<ArrowRight size={14} />}>
          ...
          <div className="mt-6 md:hidden text-center">
            <Link href="/blog" className="inline-flex items-center gap-1.5 text-brand-purple-600 font-semibold text-sm hover:gap-3 transition-all">
